...
public class P32_DistinctSubsequences {
    private static final int mod = (int)(1e9 + 7);
    public static int optimal(String s, String t)
    {
        int n = s.length();
        int m = t.length();
    ...
    public static int memoizationRecur(String s1, String s2, int ind1, int ind2, int[][] dp)
    {
        ...
        {
            int leaveOne=memoizationRecur(s1,s2,ind1-1,ind2-1,dp);
            int stay=memoizationRecur(s1,s2,ind1-1,ind2,dp);
            result=(leaveOne+stay)%mod;
        }
        else
            result=memoizationRecur(s1,s2,ind1-1,ind2,dp);
    ...
    public static int memoization(String s, String t)
    {
        ...
        for(int i=0;i<lt;i++)
            for(int j=0;j<ls;j++)
                dp[i][j]=-1;
        return memoizationRecur(s, t, lt - 1, ls - 1, dp);
    }
    public static int recursionRecur(String s1, String s2, int ind1, int ind2)
    {
        ...
        if(s1.charAt(ind1)==s2.charAt(ind2))
        {
            int leaveOne=recursionRecur(s1,s2,ind1-1,ind2-1);
            int stay=recursionRecur(s1,s2,ind1-1,ind2);
            result=(leaveOne+stay)%mod;
        }
        else
            result=recursionRecur(s1,s2,ind1-1,ind2);
        return result;
    }
    public static int recursion(String s, String t)
    {
        int lt = s.length();
        int ls = t.length();
        return recursionRecur(s, t, lt - 1, ls - 1);
    }
    public static void main(String[] args) {
        String s1 = "babgbag";
        String s2 = "bag";
        System.out.println("The Count of Distinct Subsequences is " + tabulation(s1, s2));
    }
}
